        ...
        if (mailSender != null) {
            try {
                ...
                String htmlContent = "<div style=\"background-color: #0B0F19; padding: 40px 10px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; text-align: center; color: #F3F4F6;\">"
                        ...
                        +
                        "    <img src=\"https://revynd-api-939729691035.us-east1.run.app/icon.png\" style=\"width: 64px; height: 64px; border-radius: 14px; margin-bottom: 16px;\" alt=\"REVYND Logo\" />"
                        +
                        ...
                        +
                        "    <p style=\"font-size: 16px; color: #9CA3AF; margin: 0 0 30px 0;\">Verify your email to track the vibe</p>"
                        +
                        "    <div style=\"height: 1px; background-color: #1F2937; margin-bottom: 30px;\"></div>" +
                        "    <h2 style=\"font-size: 20px; font-weight: 700; color: #FFFFFF; margin: 0 0 16px 0; text-align: center;\">Welcome to REVYND, "
                        + userName + "!</h2>" +
                        "    <p style=\"font-size: 15px; line-height: 24px; color: #E5E7EB; margin-bottom: 25px; text-align: left;\">Use the 6-digit verification code below to verify your email address and complete your registration.</p>"
                        +
                        "    <div style=\"background-color: #1F2937; border: 1px solid #374151; border-radius: 12px; padding: 18px 0; margin-bottom: 30px; letter-spacing: 6px; font-size: 32px; font-weight: bold; color: #FFFFFF; font-family: monospace;\">"
                        + code + "</div>" +
                        "    <p style=\"font-size: 14px; color: #9CA3AF; margin-bottom: 30px; text-align: left;\">⏱️ This verification code is active for <strong>15 minutes</strong>. If expired, please send a new request from the mobile application.</p>"
                        +
        ...
        if (mailSender != null) {
            try {
                ...
                String htmlContent = "<div style=\"background-color: #0B0F19; padding: 40px 10px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; text-align: center; color: #F3F4F6;\">"
                        ...
                        +
                        "    <div style=\"height: 1px; background-color: #1F2937; margin-bottom: 30px;\"></div>" +
                        "    <p style=\"font-size: 16px; line-height: 24px; color: #E5E7EB; margin-bottom: 25px; text-align: left;\">"
                        + userName
                        + ", you requested a password reset for your REVYND account. Use the 6-digit code below to authorize this change:</p>"
                        +
                        "    <div style=\"background-color: #1F2937; border: 1px solid #374151; border-radius: 12px; padding: 18px 0; margin-bottom: 30px; letter-spacing: 6px; font-size: 32px; font-weight: bold; color: #FFFFFF; font-family: monospace;\">"
                        + code + "</div>" +
                        "    <p style=\"font-size: 14px; color: #9CA3AF; margin-bottom: 30px; text-align: left;\">⏱️ This authorization code is active for <strong>15 minutes</strong>. If expired, please send a new request from the mobile application.</p>"
                        +
                        "    <div style=\"height: 1px; background-color: #1F2937; margin-bottom: 25px;\"></div>" +
                ...
                logger.error("❌ Failed to send password reset email to {} via JavaMailSender: {}", toEmail,
                        e.getMessage());
            }
        } else {
